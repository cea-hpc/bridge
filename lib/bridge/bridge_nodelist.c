...
*/
int
bridge_nodelist_equal_patterns(bridge_nodelist_t* first_list,bridge_nodelist_t* second_list)
{
  int fstatus=1;
  fstatus=bridge_nodepattern_equals(&(first_list->pattern),&(second_list->pattern));
  return fstatus;
}
...
*/
int
bridge_nodelist_non_recursive_intersects(bridge_nodelist_t* first_list,bridge_nodelist_t* second_list)
{
  ...
  same_pattern=bridge_nodelist_equal_patterns(first_list,second_list);

  /* we go further only if they represent the same node name pattern */
  if(same_pattern)
    {
      /* now we just have to test if ranges array intersect */
      fstatus=bridge_rangelist_intersects(&(first_list->rangelist),
...
*/
int
bridge_nodelist_intersects(bridge_nodelist_t* first_list,bridge_nodelist_t* second_list)
{

  int fstatus=0;
  bridge_nodelist_t* list1;
  bridge_nodelist_t* list2;
  ...
  while(list2!=NULL){
    list1=first_list;
    while(list1!=NULL){
      fstatus=bridge_nodelist_non_recursive_intersects(list1,list2);
      /* intersects! break! */
      if(fstatus)
	break;
      list1=list1->next;
...
*/
int
bridge_nodelist_non_recursive_includes(bridge_nodelist_t* first_list,bridge_nodelist_t* second_list)
{
  int fstatus=0;
  int same_pattern=0;
  ...
  if(same_pattern)
    {
      ...
      fstatus=bridge_rangelist_includes(&(first_list->rangelist),&(second_list->rangelist));
    }
  return fstatus;
}
...
*/
int
bridge_nodelist_includes(bridge_nodelist_t* first_list,bridge_nodelist_t* second_list)
{
  int fstatus=0;
  bridge_nodelist_t* list1;
  bridge_nodelist_t* list2;
  ...
  while(list2!=NULL){
    list1=first_list;
    while(list1!=NULL){
      fstatus=bridge_nodelist_non_recursive_includes(list1,list2);
...
*/
int
bridge_nodelist_add_ids(bridge_nodelist_t* nodelist,char* idlist)
{
  int fstatus=0;
  fstatus=bridge_rangelist_add_list(&(nodelist->rangelist),idlist);
  return fstatus;
}

int bridge_common_split_nodelist_entry(char* list,char** p_prefix,char** p_idlist,char** p_suffix){
  ...
  *p_suffix=suffix;
  ...
  DEBUG3_LOGGER("list=%s prefix=%s idlist=%s suffix=%s",list,
		(prefix==NULL)?"":prefix,
		(idlist==NULL)?"":idlist,
		(suffix==NULL)?"":suffix);
...
int bridge_nodelist_init(bridge_nodelist_t* nodelist,char** lists, int lists_nb){
  ...
  nodelist->next=NULL;
  fstatus+=bridge_rangelist_init(&(nodelist->rangelist));
  fstatus+=bridge_nodepattern_init(&(nodelist->pattern));
  ...
  if(fstatus==0)
      ...
      for(i=0;i<lists_nb;i++)
	{
	  ...
	  switch(operation){
	    ...
	    case 1:
	      fstatus+=bridge_nodelist_add_nodes(nodelist,list);
	      break;

	    case 2:
	      bridge_nodelist_remove_nodes(nodelist,list);
	      break;
...
int bridge_nodelist_free_contents(bridge_nodelist_t* nodelist){

  int fstatus=-1;

  if(nodelist->next!=NULL)
    bridge_nodelist_free_contents(nodelist->next);
  nodelist->next=NULL;
  bridge_nodepattern_free_contents(&(nodelist->pattern));
  bridge_rangelist_free_contents(&(nodelist->rangelist));
...
*/
int
bridge_nodelist_copy(bridge_nodelist_t* dest_list,bridge_nodelist_t* src_list)
{
  int fstatus=-1;

  bridge_nodelist_t** pwldest;
  bridge_nodelist_t** pwlsrc;

  bridge_nodelist_free_contents(dest_list);

  pwldest=&dest_list;
  if(bridge_nodelist_init(*pwldest,NULL,0)==0){
	...
	if(bridge_nodepattern_init_by_copy(&((*pwldest)->pattern),&((*pwlsrc)->pattern))!=0){
	  ...
	  if((*pwlsrc)->pattern.basic!=1){
	    // add ids
	    if(bridge_rangelist_init_by_copy(&((*pwldest)->rangelist),&((*pwlsrc)->rangelist))!=0){
...
*/
int
bridge_nodelist_is_empty(bridge_nodelist_t* nodelist){
...
*/
int
bridge_nodelist_add_nodelist(bridge_nodelist_t* nodelist,bridge_nodelist_t* second_list)
{
  int fstatus=-1;
  ...
  if(bridge_nodelist_is_empty(second_list)){
    return 0;
  }

  /* If nodelist is empty, duplicate second_list! */
  if(bridge_nodelist_is_empty(nodelist)){
    fstatus=bridge_nodelist_copy(nodelist,second_list);
  }
  /* we have to add each second list sublist to the first one */
	...
	if(bridge_nodepattern_equals((&(*pwldest)->pattern),&((*pwlsrc)->pattern))){
	  if( (*pwldest)->pattern.padding < (*pwlsrc)->pattern.padding )
	    bridge_nodepattern_set_padding(&((*pwldest)->pattern),(*pwlsrc)->pattern.padding);
	  fstatus=bridge_rangelist_add_rangelist(&((*pwldest)->rangelist),&((*pwlsrc)->rangelist));
	  break;
	}
	...
	pwldest=&((*pwldest)->next); /* increment dst sublist */
      ...
      if(*pwldest==NULL){
	*pwldest=(bridge_nodelist_t*)malloc(sizeof(bridge_nodelist_t));
	if(*pwldest!=NULL){
	  fstatus=bridge_nodelist_init(*pwldest,NULL,0);
	  if(fstatus==0){
	    fstatus=bridge_nodepattern_init_by_copy(&((*pwldest)->pattern),&((*pwlsrc)->pattern));
	    if(fstatus==0){
	      fstatus=bridge_rangelist_add_rangelist(&((*pwldest)->rangelist),&((*pwlsrc)->rangelist));
	    }
	  }
...
*/
int
bridge_nodelist_remove_nodelist(bridge_nodelist_t* nodelist,bridge_nodelist_t* second_list)
{
  int fstatus=-1;

  int add_flag;

  bridge_nodelist_t worklist;
  bridge_nodelist_t** pwldest;
  bridge_nodelist_t** pwlsrc;
  ...
  /* we have to remove each second list sublist from the first one */
    ...
    fstatus=bridge_nodelist_init(&worklist,NULL,0);
    if(fstatus==0){
	...
	while(*pwlsrc!=NULL){
	  ...
	  if(bridge_nodepattern_equals((&(*pwldest)->pattern),&((*pwlsrc)->pattern))){
	    ...
	    if((*pwldest)->pattern.basic==0){
	      fstatus+=bridge_rangelist_remove_rangelist(&((*pwldest)->rangelist),&((*pwlsrc)->rangelist));
	    }
	    else
	      fstatus=0;
	...
	if(add_flag==1){
	  fstatus+=bridge_nodelist_add_nodelist(&worklist,*pwldest);
	}
	...
	pwldest=&((*pwldest)->next); /* increment dest sublist */
      ...
      if(fstatus==0){
	fstatus=bridge_nodelist_copy(nodelist,&worklist);
      }

      bridge_nodelist_free_contents(&worklist);
    }
...
int bridge_nodelist_add_nodes(bridge_nodelist_t* nodelist,char* list){
  ...
  if(bridge_common_string_get_tokens_quantity(list," ",&token_nb)==0){
    ...
    for(i=1;i<=token_nb;i++){
      if(bridge_common_string_get_token(list," ",i,&token)==0){

	status=bridge_common_split_nodelist_entry(token,&prefix,&idlist,&suffix);
	if(status){
	  fstatus=-1;
	}
	else{
	  fstatus=bridge_nodelist_init(&wlist,NULL,0);
	  if(fstatus==0){
	    bridge_nodepattern_set_prefix(&(wlist.pattern),prefix);
	    bridge_nodepattern_set_suffix(&(wlist.pattern),suffix);
	    if(idlist!=NULL){
	      wlist.pattern.basic=0;
	      fstatus=bridge_nodelist_add_ids(&wlist,idlist);
	      bridge_nodepattern_set_padding(&(wlist.pattern),fstatus);		
	      fstatus=0;
	    }

	    fstatus=bridge_nodelist_add_nodelist(nodelist,&wlist);
	      
	    bridge_nodelist_free_contents(&wlist);
	  }
  ...
  if(bridge_common_string_get_tokens_quantity(list," ",&token_nb)==0){
	...
	if(status){
	  ...
	  if(fstatus==0){
	    ...
	    fstatus=bridge_nodelist_remove_nodelist(nodelist,&wlist);
...
int bridge_nodelist_add_nodes_range(bridge_nodelist_t* nodelist,long int from_id, long int to_id){
  ...
  fstatus=bridge_rangelist_add_range(&(nodelist->rangelist),&r);  

  return fstatus;

}

long int bridge_nodelist_non_recursive_nodes_quantity(bridge_nodelist_t* nodelist){
...
long int bridge_nodelist_nodes_quantity(bridge_nodelist_t* nodelist){
  ...
  nlist=nodelist;
  while(nlist!=NULL){
    quantity+=bridge_nodelist_non_recursive_nodes_quantity(nlist);
    nlist=nlist->next;
  }
...
int bridge_nodelist_get_extended_string(bridge_nodelist_t* nodelist,char** p_string){
  ...
  if(output_string){
	...
	if(nlist->pattern.basic==1){
	  ...
	  if(bridge_common_string_appends_and_extends(&output_string,&output_string_size,node_string_size,node_string," ")){
	    fstatus=-1;
	  }
	  else{
	    fstatus=0;
...
int bridge_nodelist_get_compacted_string(bridge_nodelist_t* nodelist,char** p_string){
  ...
  if(output_string){
    ...
    while(nlist!=NULL){
      ...
      nodes_nb=bridge_nodelist_non_recursive_nodes_quantity(nlist);
      if(nodes_nb==0){
	return fstatus;
      }
      else if(nodes_nb==1)
	...
	if(ranges_string!=NULL){
	  ...
	  /* add prefix */
	  if(prefix!=NULL)
	    bridge_common_string_appends_and_extends(&ranges_string,&ranges_string_size,strlen(prefix),prefix,"");
	  if(brackets_flag)
	    bridge_common_string_appends_and_extends(&ranges_string,&ranges_string_size,1,"[","");
	  ...
	  /* add suffix */
	  if(suffix!=NULL)
	    bridge_common_string_appends_and_extends(&ranges_string,&ranges_string_size,strlen(suffix),suffix,"");
	}
      }
	...
	break;
      ...
      if(bridge_common_string_appends_and_extends(&output_string,&output_string_size,
  ...
  if(npin->prefix!=NULL)
    {
      ...
      if(np->prefix==NULL)
	{
	  bridge_nodepattern_free_contents(np);
	  return fstatus;
	}
    }
  if(npin->suffix!=NULL)
