/*****************************************************************************\
 *  plugins/batch/lsf_slurm/lsf_batch_session.c - 
 *****************************************************************************
...
#define LSF_WORKDIR "/usr/local/lsf/work"
...
int get_batch_sessions(bridge_batch_manager_t* p_batch_manager,
  ...
  if(jobs_number<0)
	...
	if(p_jobInfo==NULL)
	  {
	    DEBUG3_LOGGER("unable to extract job (%d/%d) information from LSF data\n",i+1,jobs_number);
	...
	else
	  {
	    ...
	    /* set batch session executing hostname */
	    ...
	    /* set batch session memory usage */
	    (*p_p_batch_sessions)[stored_jobs_number].seq_mem_used=(uint32_t)floor(p_jobInfo->runRusage.swap/1024);
	    ...
	    /* we finaly set sequential limit using parallel limit (LSF-Slurm doesn't support the 2 sorts) */
	    ...
	    char* pointer;
	    char* addinfos;
	    addinfos=strdup(p_jobInfo->additionalInfo);
	    if(addinfos!=NULL)
	      {
		pointer=strchr(addinfos,';');
		if(pointer!=NULL)
		  *pointer='\0';
		pointer=strchr(addinfos,'=');
		if(pointer!=NULL)
		  {
		    pointer++;
		    (*p_p_batch_sessions)[stored_jobs_number].rm_id=strdup(pointer);
		  }
		free(addinfos);
	      }
	...
	for(i;i<jobs_number;i++){
	  ...
	  if(lsb_acct_record==NULL || lsb_acct_record->type!=EVENT_JOB_FINISH)
	    ...
	    */
	    ...
	    /* 	  break; */
	    /* 	case BRIDGE_BATCH_SESSION_STATE_PENDING : */
	    /* 	  reason=lsb_pendreason(1,&p_jobInfo->reasons,NULL,NULL,0); */
	    /* 	  if(reason!=NULL) */
	    /* 	    reason[strlen(reason)-2]='\0'; */
	    /* 	  (*p_p_batch_sessions)[stored_jobs_number].reason=strdup(reason); */
	    /* 	  break; */
	    ...
	    // unavailable
	    (*p_p_batch_sessions)[stored_jobs_number].seq_mem_used=(uint32_t)floor(p_jobInfo->lsfRusage.ru_maxrss/1024);
	     ...
	     * -----------------------------
	     */
	    if(p_jobInfo->maxRMem!=0)
	      (*p_p_batch_sessions)[stored_jobs_number].par_mem_used=p_jobInfo->maxRMem/1024;
	    (*p_p_batch_sessions)[stored_jobs_number].par_cores_nb=p_jobInfo->numExHosts;
	    (*p_p_batch_sessions)[stored_jobs_number].par_time_used=(*p_p_batch_sessions)[stored_jobs_number].system_time+(*p_p_batch_sessions)[stored_jobs_number].user_time;
	    char* par_nodelist=strrchr(p_jobInfo->additionalInfo,'=');
	    if(par_nodelist!=NULL)
	      bridge_nodelist_add_nodes(&((*p_p_batch_sessions)[stored_jobs_number].par_nodelist),par_nodelist+1);

	    /*
	     * On retraite les donnees
	     */
	    if((*p_p_batch_sessions)[stored_jobs_number].par_cores_nb!=0){
	      (*p_p_batch_sessions)[stored_jobs_number].par_mem_used=(*p_p_batch_sessions)[stored_jobs_number].seq_mem_used;
	      (*p_p_batch_sessions)[stored_jobs_number].par_time_used=(*p_p_batch_sessions)[stored_jobs_number].par_time_used/(*p_p_batch_sessions)[stored_jobs_number].par_cores_nb;
	      (*p_p_batch_sessions)[stored_jobs_number].system_time=0;
	      (*p_p_batch_sessions)[stored_jobs_number].user_time=0;
	      (*p_p_batch_sessions)[stored_jobs_number].seq_mem_used=0;
	    }
	    else{
	      (*p_p_batch_sessions)[stored_jobs_number].par_mem_used=0;
	      (*p_p_batch_sessions)[stored_jobs_number].par_time_used=0;
	    }
